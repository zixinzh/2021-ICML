...
gapbd = C_budget*L*8*floor( log(L)/log(u) ) / T;
...
for m = 1 : M
    ...
    item_pull = repmat( 1:Size_act_set, n_m, 1 );
    item_pull = reshape( item_pull, 1, n_m*Size_act_set );          % loc of the items to pull: 1,1,2,2,...  
    ...
    if (C_budget >= 1) && ( item_optimal_active  )
        ...
        step_pull_item_not_opt = find(  item_pull ~= index_opt_in_cur_act );                         % time steps when optimal item is not pulled     
        C_subset(  step_pull_item_not_opt ) = 0; % corruption = 0 when optimal item is not pulled
    ...
    W_subset = W_subset(1:n_m*Size_act_set, : ); 
